...
public extension View {
    private func newWindowInternal(title: String,
                                   geometry: NSRect,
                                   shouldCenter: Bool = true,
                                   style: NSWindow.StyleMask,
                                   delegate: NSWindowDelegate,
                                   windowEscape: ((NSWindow) -> Void)? = nil) -> NSWindow {
        let window = NSWindow(
    ...
    func openNewWindow(title: String,
                       ...
                       windowEscape: ((NSWindow) -> Void)? = nil) {
        newWindowInternal(title: title, geometry: geometry, shouldCenter: shouldCenter, style: style, delegate: delegate, windowEscape: windowEscape).contentView = NSHostingView(rootView: self)
    }
}
#endif
